...
def get_recommendations_item_to_item(top_m, discription, brons, n_ids):
    ...
    user_items = [item[item['id'] == 1]['index'].iloc[0],
                  item[item['id'] == 2]['index'].iloc[0],
                  item[item['id'] == 3]['index'].iloc[0]]
    num_items = 10000
    sparse_matrix_for_user = create_sparse_matrix_for_user(user_items, num_items)
    print(user_items, num_items)
    print(sparse_matrix_for_user)
    print(sparse_matrix_for_user.dtype)
    sparse_matrix_for_user_int = sparse_matrix_for_user.astype(int)
    print(sparse_matrix_for_user.dtype)

    # Вызов функции recommend() с преобразованной матрицей
    raw_recs = model.recommend(0, sparse_matrix_for_user, N=15, filter_already_liked_items=False, recalculate_user=True)
